...
cd /home/hpillai/release_8.1.0/pgi
...
export ESMF_MPIRUN=/home/hpillai/release_8.1.0/pgi/esmf/src/Infrastructure/stubs/mpiuni/mpirun
...
if [ "$build" == "1" ]; then
  ...
  cp -rf $ESMF_DIR/tmp  /home/hpillai/release_8.1.0/store/build_$ESMF_OS$ESMF_ABI$ESMF_COMPILER$ESMF_COMM.out
fi
...
then
  ...
  #cp -rf $ESMF_DIR/tmp  /home1/07356/tg866552/release_8.1.0/store/build_$OPT_$ESMF_OS$ESMF_ABI$ESMF_COMPILER$ESMF_COMM.out
  echo "|------------------------ INSTALL -------------------------------|"

  gmake -j32 ESMF_BOPT=$OPT  install 1> tmp 2>&1
  cp -rf $ESMF_DIR/tmp  /home/hpillai/release_8.1.0/store/install_$ESMF_OS$ESMF_ABI$ESMF_COMPILER$ESMF_COMM.out
  if [ "$?" != "0" ]
  then
    ...
    cp -rf $ESMF_DIR/tmp  /home/hpillai/release_8.1.0/store/installcheck_$ESMF_OS$ESMF_ABI$ESMF_COMPILER$ESMF_COMM.out
    if [ "$?" != "0" ]
    then
    ...
    if [ "$build" == "1" ]; then
      ...
      cp -rf $ESMF_DIR/tmp  /home/hpillai/release_8.1.0/store/quickstartbuild_$ESMF_OS$ESMF_ABI$ESMF_COMPILER$ESMF_COMM.out
    fi
  ...
  if [ $do_apps == 1 ]; then
    ...
    cp -rf $ESMF_DIR/tmp  /home/hpillai/release_8.1.0/store/build_apps_$ESMF_OS$ESMF_ABI$ESMF_COMPILER$ESMF_COMM.out
    if [ "$?" != "0" ]
    then
...
echo "RUN: [$RUN]"
  ...
  if [ $do_sys == 1 ]; then
    ...
    cp -rf $ESMF_DIR/tmp  /home/hpillai/release_8.1.0/store/build_system_tests_$ESMF_OS$ESMF_ABI$ESMF_COMPILER$ESMF_COMM.out
    if [ "$?" != "0" ]
    then
    ...
    cp -rf $ESMF_DIR/tmp  /home/hpillai/release_8.1.0/store/run_system_tests_uni_$ESMF_OS$ESMF_ABI$ESMF_COMPILER$ESMF_COMM.out
  else
    make run_system_tests 1> tmp 2>&1
    cp -rf $ESMF_DIR/tmp  /home/hpillai/release_8.1.0/store/run_system_tests_$ESMF_OS$ESMF_ABI$ESMF_COMPILER$ESMF_COMM.out
  fi
  ...
  if [ $do_examples == 1 ]; then
    ...
    cp -rf $ESMF_DIR/tmp  /home/hpillai/release_8.1.0/store/build_examples_$ESMF_OS$ESMF_ABI$ESMF_COMPILER$ESMF_COMM.out
    if [ "$?" != "0" ]
    then
    ...
    cp -rf $ESMF_DIR/tmp  /home/hpillai/release_8.1.0/store/run_examples_uni_$ESMF_OS$ESMF_ABI$ESMF_COMPILER$ESMF_COMM.out
  else
    make run_examples 1> tmp 2>&1
    cp -rf $ESMF_DIR/tmp  /home/hpillai/release_8.1.0/store/run_examples_$ESMF_OS$ESMF_ABI$ESMF_COMPILER$ESMF_COMM.out
  fi
  ...
  if [ $do_unit == 1 ]; then
    ...
    cp -rf $ESMF_DIR/tmp  /home/hpillai/release_8.1.0/store/build_unit_tests_$ESMF_OS$ESMF_ABI$ESMF_COMPILER$ESMF_COMM.out
    ...
    cp -rf $ESMF_DIR/tmp  /home/hpillai/release_8.1.0/store/run_unit_tests_uni_$ESMF_OS$ESMF_ABI$ESMF_COMPILER$ESMF_COMM.out
  else
    make run_unit_tests 1> tmp 2>&1
    cp -rf $ESMF_DIR/tmp  /home/hpillai/release_8.1.0/store/run_unit_tests_$ESMF_OS$ESMF_ABI$ESMF_COMPILER$ESMF_COMM.out
  fi

    #cp -rf $ESMF_DIR/tmp  /home1/07356/tg866552/release_8.1.0/store/run_unit_tests_$ESMF_OS$ESMF_ABI$ESMF_COMPILER$ESMF_COMM.out
    # if !(grep "passed" $LOGDIR/queue_results)
    # then
    #         # Set test status as yellow, if it is not red
    #         if !(grep "red" $LOGDIR/Test_Status)
...
then  #If the library build or the unit tests build failed,  don't run benchmarking
  ...
  then
    ...
    cp -rf $ESMF_DIR/tmp  /home/hpillai/release_8.1.0/store/install_unit_tests_benchmark_$ESMF_OS$ESMF_ABI$ESMF_COMPILER$ESMF_COMM.out
    if [ "$?" != "0" ]
    then
  ...
  cp -rf $ESMF_DIR/tmp  /home/hpillai/release_8.1.0/store/run_unit_tests_benchmark_$ESMF_OS$ESMF_ABI$ESMF_COMPILER$ESMF_COMM.out
  if [ "$?" != "0" ]
  then
...
cd /home/hpillai/release_8.1.0/store
...
mail -s "Release_8.1.0_Orion_pgi" himanshu@ucar.edu esmf_test@cgd.ucar.edu < $LOGDIR/newMailMessage
