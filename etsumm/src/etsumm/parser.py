...
class Parser(object):
    ...
    def iter_test_meta(self):
        ...
        for test_target, target_meta in TARGET_META.items():
            # Loop for each test target: examples, unit_tests, system_tests
            target_dir = os.path.join(self.results_dir, target_meta['dir'])
            ...
            for de in os.scandir(target_dir):
                ...
                if de.name.endswith(target_meta['suffix']):
    ...
    def iter_test_runners(cls, xmlout, verbosity=1):
        for parser in cls.iter_parsers():
            log('Current Parser.config={}'.format(parser.config))
