...
export homedir=/scratch/esmf/esmf_test/cronjobs/trunk/intel
...
$homedir/bin/chk_out_esmf $rfile ESMF_Bighorn_intel
...
do
	...
	export THREADED=$threaded
        export ESMF_INSTALL_PREFIX=/scratch/esmf/esmf_test/cronjobs/trunk/intel/install_dir
	...
	#export MODULEPATH=$MODULEPATH":/project/esmf/modulefiles"
        ...
        then
            ...
            module load tool/hdf5/1.8.7/intel 
        ...
        then
             ...
             #module load mpi/intel/mvapich2-2.1-qlc
	     module list
        fi
	...
	if [[ $ESMF_COMM = "openmpi" ]]
        then
	        ...
	        module load mpi/intel/openmpi-1.10.7-qlc
		module list
        fi
...
cat /scratch/esmf/esmf_test/cronjobs/trunk/intel/esmf_logs/*/Test_Status >> meta_data_bighorn_intel.log
echo "Test_Failures :" `cat /scratch/esmf/esmf_test/cronjobs/trunk/intel/esmf_logs/*/Test_Failures` >> meta_data_bighorn_intel.log
echo "Test_artifacts made at" `date` >> meta_data_bighorn_intel.log

#cd /scratch/esmf/esmf_test/cronjobs/trunk/intel/esmf_logs/2011_test/
#cp -rf ESMFdailyLog /home/himanshu/storage/develop/ESMF_Trunk_Bighorn_intel.html
#sed -i '1i<pre>' /home/himanshu/storage/develop/ESMF_Trunk_Bighorn_intel.html

cd ~/storage/esmf-test-artifacts/
git pull


cd ~/storage/esmf-test-artifacts/develop/bighorn/intel/19.0.2.187/g/mpiuni/apps/
touch c.sh
git rm *.stdout
git rm *.Log
...
cd ~/storage/esmf-test-artifacts/develop/bighorn/intel/19.0.2.187/O/mpiuni/apps/
touch d.sh
git rm *.stdout
git rm *.Log
...
cd ~/storage/esmf-test-artifacts/develop/bighorn/intel/19.0.2.187/g/mpiuni/lib/
touch a.sh
git rm *.mk
cp -rf ~/storage/develop/libg_Linux_intel_19.0.2.187_mpiuni/* .
rm a.sh

cd ~/storage/esmf-test-artifacts/develop/bighorn/intel/19.0.2.187/O/mpiuni/lib/
touch a.sh
git rm *.mk
...
cd ~/storage/esmf-test-artifacts/develop/bighorn/intel/19.0.2.187/g/mpiuni/examples/
touch a.sh
git rm *.config
git rm *_results
git rm *.stdout
...
cd ~/storage/esmf-test-artifacts/develop/bighorn/intel/19.0.2.187/O/mpiuni/examples/
touch a.sh
git rm *.config
git rm *_results
git rm *.stdout
...
cd ~/storage/esmf-test-artifacts/develop/bighorn/intel/19.0.2.187/g/mpiuni/test
touch a.sh
git rm *.config
git rm *_results
git rm *.stdout
...
cd ~/storage/esmf-test-artifacts/develop/bighorn/intel/19.0.2.187/O/mpiuni/test
touch a.sh
git rm *.config
git rm *_results
git rm *.stdout
...
cd ~/storage/esmf-test-artifacts/develop/bighorn/intel/19.0.2.187/g/mpiuni/out/
touch d.sh
git rm *.out
...
cd ~/storage/esmf-test-artifacts/develop/bighorn/intel/19.0.2.187/O/mpiuni/out/
touch d.sh
git rm *.out
...
cd ~/storage/esmf-test-artifacts/develop/bighorn/intel/
touch a.sh
git rm summary.log
#git rm *.html
cp -rf ~/storage/develop/meta_data_bighorn_intel.log summary.log
#cp -rf ~/storage/develop/ESMF_Trunk_Bighorn_intel.html result.html
rm a.sh

cd /home/himanshu/storage/esmf-test-artifacts
#git log --since=6.days --grep='Bighorn Intel artifacts' >  ~/storage/esmf-test-artifacts/develop/bighorn/intel/commit.log

git add .
git commit -m " Bighorn Intel artifacts pushed at `date`"
