                ...
                for ver in machine_list[comp]["versions"]:
                    ...
                    for key in mpitypes:
                        for branch in machine_list["branch"]:
                            ...
                            os.chdir(subdir)
                            os.system(
                                "rm -rf *.e *.o *bat.e* *bat.o* *.log DEFAULTINSTALLDIR nuopc-app-prototypes"
                            )
                            os.system(
                                "git remote update; git checkout {}".format(branch)
                            )
                            os.system("git pull origin {}".format(branch))
                            status = (
                                subprocess.check_output(nuopcclone, shell=True)
                                .strip()
                                .decode("utf-8")
                            )
                            print(
                                "status from nuopc clone command {} was {}".format(
                                    nuopcclone, status
                                )
                            )
                            filename = "build-{}_{}_{}_{}.bat".format(
                            ...
                            if "unloadmodule" in machine_list[comp]:
                                fb.write(
                                    "\nmodule unload {}\n".format(
                                        machine_list[comp]["unloadmodule"]
                                    )
                                )
                                ft.write(
                                    "\nmodule unload {}\n".format(
                            ...
                            if "mpi_env_vars" in mpidict[key]:
                                for mpi_var in mpidict[key]["mpi_env_vars"]:
                                    fb.write(
                                        "export {}\n".format(
                                            mpidict[key]["mpi_env_vars"][mpi_var]
                                        )
                                    )
                                    ft.write(
                                        "export {}\n".format(
                            ...
                            if "extra_env_vars" in machine_list[comp]["versions"][ver]:
                                for var in machine_list[comp]["versions"][ver][
                                    "extra_env_vars"
                                ]:
                                    fb.write(
                                        "export {}\n".format(
                            ...
                            if "extra_commands" in machine_list[comp]["versions"][ver]:
                                for cmd in machine_list[comp]["versions"][ver][
                                    "extra_commands"
                                ]:
                                    fb.write(
                                        "{}\n".format(
                            ...
                            if scheduler == "slurm":
                                ...
                                print(
                                    "Submitting test_batch with command: {}".format(
                                        batch_test
                                    )
                                )
                                jobnum = (
                                ...
                                # submit the second job to be dependent on the first
                                batch_test = "qsub -W depend=afterok:{} {}".format(
                                    jobnum, t_filename
                                )
                                print(
                                    "Submitting test_batch with command: {}".format(
