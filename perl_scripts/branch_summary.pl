...
foreach(@gitlines) {
  ...
  if($_ =~ /->/) {
    ...
    print("command is $command\n");
    system("$command");
    system("echo \"HOST compilier version mpi-type mpi-ver O/g unit-pass unit-fail sys-pass sys-fail ex-pass ex-fail\" > summary");
    system("paste -d \" \" unit sys examp >> summary");
